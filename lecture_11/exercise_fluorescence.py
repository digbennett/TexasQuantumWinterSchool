...
fluor_t2 = 500                              # t2 waiting time [fs]
...
def generate_alt_energy_spectroscopy_hamiltonian(nsites, V, E_A, E_B):
    ...
    # Add alternating on-site energies on the excited-state diagonal (skip ground index 0)
    # Sites 1..nsites: 1->E_A, 2->E_B, 3->E_A, 4->E_B, ...
    diag_rows = []
    diag_cols = []
    diag_vals = []
...
# DyadicSpectra expects n_state_hilb = n_chromophore + 1 (index 0 = ground)
# For spectroscopy with nsite chromophores, nchrom = nsite
nchrom = nsite
...
# Single Drude-Lorentz expansion per site
list_modes = bcf_convert_dl_to_exp(e_lambda, gamma, temp)

# Standard spectroscopy L-operators (diagonal projectors on excited states)
list_loperators = generate_spectroscopy_loperators(nsite)

# ---------------------------------------------------------------------
# Convergence Parameters
# ---------------------------------------------------------------------
ntraj = 100                         # Number of trajectories in ensemble
kmax = 2                                   # Hierarchy truncation depth
dt = 0.5                                      # Simulation time step [fs]
da = 0.01                            # Auxiliary adaptivity error bound
ds = 0.01                                # State adaptivity error bound
upstep = 10                           # Steps between adaptivity checks

# ---------------------------------------------------------------------
# Monte Carlo Site-Sampling Parameters (for fluorescence)
# ---------------------------------------------------------------------
site_sampling = True                         # If True, randomly select initial site per trajectory
site_seed_base = 6000                        # Base seed for site RNG (independent of dynamics RNG)
site_weights = None                          # Optional probability weights over sites (length = nsite); None -> uniform

# ---------------------------------------------------------------------
# Prepare Spectroscopy Dictionaries
# ---------------------------------------------------------------------
# Time dictionary for fluorescence (uses t_2 and t_3)
# Expose t_2 via parameter control center; keep t_3 fixed here but can be edited below if needed.
fluor_t3 = 500
fluor_time_dict = {"t_2": fluor_t2, "t_3": fluor_t3}

# Field and site definitions (match timing scripts)
E_vec = np.array([0.0, 0.0, 1.0])
field_dict_fluor = {"E_1": E_vec, "E_sig": E_vec}
...
# ---------------------------------------------------------------------
print("Running ensemble fluorescence calculation (linear chain, alternating site energies)...")
...
for i in range(ntraj):
    ...
    # Monte Carlo sample initial site (1..nsite) for this trajectory
    if site_sampling:
        ...
        if site_weights is not None:
            ...
            site_i = int(rng_site.choice(np.arange(1, nsite + 1), p=probs))
        else:
            site_i = int(rng_site.integers(1, nsite + 1))
    else:
        site_i = 1  # default fixed site

    list_sampled_sites.append(site_i)

    # Build spectroscopy input with sampled ket and bra site (use same site for both)
    site_dict_fluor_i = {"list_ket_sites": [site_i], "list_bra_sites": [site_i]}
    spec_input_fluor_i = prepare_spectroscopy_input_dict(
        "FLUORESCENCE", fluor_time_dict, field_dict_fluor, site_dict_fluor_i
    )

    # Run trajectory with deterministic dynamics seed
    dyad_fluor = DHOPS(spec_input_fluor_i, chromophore_input, convergence_dict, dyn_seed)
    R_t_fluor = np.asarray(dyad_fluor.calculate_spectrum())
    list_R_fluor.append(R_t_fluor)

# Diagnostics: show counts of sampled sites
if site_sampling:
    _u, _c = np.unique(list_sampled_sites, return_counts=True)
    print("Sampled initial site counts:", dict(zip(_u.tolist(), _c.tolist())))

# Ensemble average
len_fluor = len(list_R_fluor[0])
R_fluor_avg = np.mean(np.vstack([arr[:min_len_fluor] for arr in list_R_fluor]), axis=0)

# ---------------------------------------------------------------------
# FFT to spectrum using zero-padded FFT and plot
# ---------------------------------------------------------------------
S_fluor, w_fluor = zeropadded_fft(R_fluor_avg, dt)

plt.figure(figsize=(8, 5))
plt.plot(w_fluor, S_fluor / np.max(np.abs(S_fluor)))
plt.xlabel("Frequency (cm$^{-1}$)")
plt.ylabel("Intensity")
plt.xlim(-1000, 1000)
plt.title(f'Ensemble-Averaged Fluorescence Spectrum ({ntraj} Trajectories)')
plt.tight_layout()
plt.show()
